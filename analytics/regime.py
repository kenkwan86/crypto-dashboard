...
[-3, 3]. Positive = hot/euphoric positioning, negative = washed-out. Components:
  funding_z   - median 90d funding z across the universe (crowded longs when high)
...
def compute_regime() -> pd.DataFrame:
    funding = funding_zscores()["z90"].median(axis=1)
    ...
    breadth_pct = (closes > ma_50d)[valid].sum(axis=1) / valid.sum(axis=1)
    ...
    components = pd.DataFrame({
        "funding_z": funding, "oi_z": oi, "momentum_z": momentum,
        "breadth": breadth, "dvol_pct": dvol_score,
    }).clip(-3, 3)
    components["regime"] = components.mean(axis=1, skipna=True)
    components["n_components"] = components[["funding_z", "oi_z", "momentum_z", "breadth", "dvol_pct"]].notna().sum(axis=1)
